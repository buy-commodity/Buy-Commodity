...
export default function UserDashboard() {
    ...
    useEffect(() => {
        ...
        setDisplayData(filteredData)
        setCurrentPage(1)
    }, [filters, searchQuery, allData, sortConfig]) // No sortData dependency in this code
    ...
    const generatePDF = async (gstin: string) => {
        ...
        try {
            ...
            // Current date (September 10, 2025, 11:13 PM IST)
            const currentDate = new Date('2025-09-10T23:13:00+05:30');
            const oneYearAgo = new Date(currentDate);
            oneYearAgo.setFullYear(currentDate.getFullYear() - 1);

            const lastYearRecords = items.filter(item => {
                const itemYear = parseInt(item.year || "0", 10);
                const itemMonth = parseInt(item.month || "1", 10) - 1;
                const itemDate = new Date(itemYear, itemMonth);

                return itemDate >= oneYearAgo && itemDate <= currentDate;
            });

            lastYearRecords.sort((a, b) => {
                const yearA = parseInt(a.year || "0", 10);
                const yearB = parseInt(b.year || "0", 10);
                const monthA = parseInt(a.month || "0", 10);
                const monthB = parseInt(b.month || "0", 10);
            ...
            const prepareTableData = (records: Company[]) =>
                records.map((item) => [
                    item.year || "N/A",
                    getMonthName(item.month || "N/A"),
                    item.return_type || "N/A",
            ...
            const gstr3bRecords = lastYearRecords.filter((item) => item.return_type === "GSTR3B");
            const otherRecords = lastYearRecords.filter((item) => item.return_type !== "GSTR3B");
                ...
                <Table>
                    <TableHeader>
                        <TableRow>
                            ...
                            <TableHead onClick={() => handleSort('Delay_days')} className="cursor-pointer">
                                <div className="flex items-center">
                                    Delay Days %
                                    {sortConfig?.key === 'Delay_days'
                                        ? (sortConfig.direction === 'ascending' ? <ArrowUp className="ml-2 h-4 w-4" /> : <ArrowDown className="ml-2 h-4 w-4" />)
                        ...
                        {currentItems.map((item) => (
                            <TableRow key={item.id}>
                                ...
                                <TableCell>
                                    <Button variant="outline" size="sm" onClick={() => generatePDF(item.gstin || '')} className="mr-2" type="submit" disabled={isLoading}>
                                        {isLoading ? (
                                            <Image src="/gif/loading.gif" alt="Loading..." width={64} height={24} className="w-16 h-6" />
                                        ) : (
                                            "Download"
                                        )}
                                    </Button>
                                    <Dialog>
                                        ...
                                        <DialogContent className="sm:max-w-[425px]">
                                            ...
                                            <div className="flex justify-end space-x-4">
                                                <Button type="submit" disabled={isLoading} onClick={handleSaveChanges}>
                                                    {isLoading ? (
                                                        <Image src="/gif/loading.gif" alt="Loading..." width={24} height={24} className="w-6 h-6" />
                                                    ) : (
                                                        "Save Changes"
                                                    )}
                                                </Button>

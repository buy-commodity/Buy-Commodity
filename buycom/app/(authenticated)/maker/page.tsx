...
export default function AdminDashboard() {
    ...
    const generatePDF = async (gstin: string) => {
        ...
        try {
            ...
            // Filter ONLY GSTR3B records
            const gstr3bItems = items.filter(item => item.return_type === "GSTR3B");
            
            if (gstr3bItems.length === 0) {
                console.error("No GSTR3B records found for the provided GSTIN");
                setIsLoading(false);
                return;
            }
            
            console.log("GSTR3B Items :", gstr3bItems);

            // Deduplicate GSTR3B records based on key fields
            const uniqueKey = (item: CompanyData) =>
                `${item.year}-${item.month}-${item.return_type}-${item.date_of_filing}-${item.return_period || ''}`;
            const uniqueGSTR3BItems = Array.from(new Map(gstr3bItems.map(item => [uniqueKey(item), item])).values());
            
            console.log("uniqueGSTR3BItems :", uniqueGSTR3BItems);
            ...
            // Filter GSTR3B records by allowed financial years and months
            const filteredGSTR3BItems = uniqueGSTR3BItems.filter(item => {
                const year = parseInt(item.year || "0", 10);
                const month = parseInt(item.month || "0", 10);
            ...
            if (filteredGSTR3BItems.length === 0) {
                console.error("No relevant GSTR3B records found after applying financial year filter");
                setIsLoading(false);
                return;
            }
            ...
            doc.setFontSize(24);
            doc.setFont('bold');
            doc.text("GSTR3B Due Diligence Report", 50, 15);
            doc.setLineWidth(0.5);
            doc.line(50, 18, 160, 18);
            doc.setFontSize(10);
    
            // Summary calculations for GSTR3B only
            const delayedCount = filteredGSTR3BItems.filter(item => item.delayed_filling === "Yes").length;
            const total = filteredGSTR3BItems.length;
            const percent = total > 0 ? ((delayedCount / total) * 100).toFixed(1) + "%" : "0%";
            const avgDelay = total > 0 ? (filteredGSTR3BItems.reduce((sum, item) => sum + parseFloat(item.Delay_days || "0"), 0) / total).toFixed(1) : "0";
    
            // Summary table data for GSTR3B
            const summaryTableData = [
                ["GSTIN", filteredGSTR3BItems[0].gstin || "N/A", "STATUS", filteredGSTR3BItems[0].return_status || "N/A"],
                ["LEGAL NAME", filteredGSTR3BItems[0].legal_name || "N/A", "REG. DATE", filteredGSTR3BItems[0].registration_date || "N/A"],
                ["TRADE NAME", filteredGSTR3BItems[0].trade_name || "N/A", "LAST UPDATE DATE", filteredGSTR3BItems[0].last_update || "N/A"],
                ["COMPANY TYPE", filteredGSTR3BItems[0].company_type || "N/A", "STATE", filteredGSTR3BItems[0].state || "N/A"],
                ["% DELAYED GSTR3B", percent, "AVG. GSTR3B DELAY", avgDelay],
                ["Address", filteredGSTR3BItems[0].address || "N/A", "Result", filteredGSTR3BItems[0].result || "N/A"],
            ];
            ...
            // Helper function to prepare GSTR3B table data
            const prepareGSTR3BTableData = (records: CompanyData[]) =>
                records.map((item) => [
                    item.year || "N/A",
                    getMonthName(item.month || "N/A"),
                    item.return_period || "N/A",
            ...
            // Prepare GSTR3B table data
            const gstr3bTableData = prepareGSTR3BTableData(filteredGSTR3BItems);
    
            // Add GSTR3B records table
            if (gstr3bTableData.length > 0) {
                doc.setFontSize(16);
                doc.setFont('bold');
                doc.text("GSTR3B Records", 14, yPos);
                doc.setFontSize(10);
                doc.setFont('normal');
            ...
            // Save the PDF with GSTR3B suffix
            doc.save(`${gstin}_GSTR3B_summary.pdf`);
        } catch (error) {
            console.error("Error generating GSTR3B PDF:", error);
        } finally {
            setIsLoading(false);
        }
    };
